...
void setup() {
	...
	httpServer.on("/wakeup", wakeUp);
	httpServer.on("/reset-roomba", resetRoomba);
	httpServer.on("/start", request_startCleaning);
	httpServer.on("/stop", request_stopCleaning);
	httpServer.on("/dock", request_return_to_base);	
...
void sendState() {
	...
	client.publish(topic_state, output.c_str(), true);
}
...
void handle_root() {
	...
	String webpage = String("<!DOCTYPE HTML>") +
		F("<head><title>Roomba Status</title><link href='https://fonts.googleapis.com/css?family=Lato' rel='stylesheet'></head>") +
		F("<style>html{border-top:7px solid #64a70b}body,html{background-color:#efefef;margin:0;padding:0;font-family:Lato;font-size:1em}h1{padding:0 20%;text-align:right;background-color:#fff;margin:0;height:2em;line-height:2em;font-size:1.5em;background-image:url(https://upload.wikimedia.org/wikipedia/commons/thumb/1/13/IRobot_Green_logo.png/320px-IRobot_Green_logo.png);background-repeat:no-repeat;background-size:4em 2.2em;background-position:20% 0}table{background-color:#fff;margin:2em 20%;width:60%;font-size:.8em;border:0}table td{padding:5px 20px;border-color:#efefef}table tr td:first-child{background-color:#64a70b;color:#fff}h2{margin:2em 20%;width:60%;font-size:1.2em}ul{text-align:center}pre,ul{margin:0 20%;width:60%;font-size:1;list-style-type:none;padding:0}li{border:2px solid #efefef;overflow:auto;display:inline-block}li a{text-decoration:none;color:#000;background-color:#fff;padding:1em;height:1em;display:block}li a:hover{background-color:#64a70b;color:#fff}</style>") +
		F("<body><h1>Roomba Status</h1>") +
		F("<h2>Actions</h2>") +
		F("<ul>") +
		F("<li><a href=\"javascript:fetch('/wakeup')\">Wake up</a></li>") + 
		F("<li><a href=\"javascript:fetch('/start')\">Start cleaning</a></li>") + 
		F("<li><a href=\"javascript:fetch('/stop')\">Stop cleaning</a></li>") + 
		F("<li><a href=\"javascript:fetch('/dock')\">Back to dock</a></li>") + 
		F("<li><a href=\"javascript:fetch('/reset-roomba')\">Reset Roomba</a></li>") + 
		F("<li><a href=\"/reboot\">Reboot controller</a></li>") +
		F("<li><a href=\"/update\">Update controller</a></li>") + 
		F("</ul>") +
		F("<h2>Last Status</h2>") +
		F("<table border='1' rules='rows' cellpadding='5'>") +
		...
		F("</table>") +
		F("<h2>Status Log</h2>") +
		F("<pre>") + status_log + F("</pre>") + 
		F("<h2>Update Status</h2>") + 
		F("<pre>") + update_status + F("</pre><hr>") + 
		F("<font size=\"-1\">") + ESP.getFullVersion() + F("</font><br></body></html>");
...
void wakeUp(){
	String webpage = "<html><head><meta http-equiv=\"refresh\" content=\"10;url=/\"></head><body>Waking up</body></html>";
	httpServer.send(200, "text/html", webpage);
	StayAwake();
}

void request_startCleaning() {
	startCleaning();
